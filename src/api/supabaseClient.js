...
export function clearSupabaseSession() { window.localStorage.removeItem(sessionKey); }
...
export const supabaseAuth = {
  ...
  async signUp(email, password, data = {}) {
    const result = await request('/auth/v1/signup', {
      method: 'POST',
      body: JSON.stringify({ email, password, data, options: { emailRedirectTo: window.location.origin + '/auth' } }),
    });
  ...
  async resetPasswordForEmail(email) {
    return request('/auth/v1/recover', {
      method: 'POST',
      body: JSON.stringify({ email, options: { redirectTo: window.location.origin + '/reset-password' } }),
    });
  },
...
export async function uploadProfilePhoto(file) {
  ...
  const response = await fetch(baseUrl + '/storage/v1/object/profile-photos/' + path, {
    method: 'POST',
    headers: { apikey: publishableKey, Authorization: 'Bearer ' + session.access_token, 'Content-Type': file.type || 'image/jpeg', 'x-upsert': 'false' },
    body: file,
  });
  const body = await response.json().catch(() => null);
