// Minimal Supabase HTTP client used during the Base44-to-Supabase migration.
// It deliberately uses only the publishable browser key; service-role keys
...
function requireConfig() {
  if (!baseUrl || !publishableKey) {
    throw new Error('Nmood is not connected to Supabase yet.');
  }
}
...
// Supabase email confirmation and password-recovery links deliver credentials
// in the URL fragment. Store them once, then remove the sensitive fragment
// from browser history before the rest of the application starts.
...
async function request(path, options = {}) {
  ...
  if (!response.ok) throw new Error(body?.message || body?.error_description || 'Supabase request failed.');
  return body;
}
  ...
  async signUp(email, password, data = {}) {
    const result = await request('/auth/v1/signup', {
      ...
      body: JSON.stringify({
        ...
        password,
        data,
        // Supabase sends the verification email back to the independently
        // hosted Nmood origin; no Base44 return URL is involved.
...
export async function uploadProfilePhoto(file) {
  ...
  const response = await fetch(`${baseUrl}/storage/v1/object/profile-photos/${path}`, {
    method: 'POST',
    headers: {
      apikey: publishableKey,
