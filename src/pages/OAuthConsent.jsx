...
// Inline consent layout (replaces the deleted ConsentLayout component).
function ConsentLayout({ title, subtitle, children }) {
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-background via-background to-primary/5 p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-6">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-button bg-nmood-gradient shadow-elevated mb-3">
            <ShieldCheck className="w-6 h-6 text-white" />
          </div>
          <h1 className="font-heading text-2xl font-bold tracking-tight text-foreground text-balance">{title}</h1>
          {subtitle && <p className="text-xs text-muted-foreground mt-1 leading-relaxed">{subtitle}</p>}
        </div>
        <div className="rounded-card border border-border/50 bg-card/80 backdrop-blur-xl shadow-elevated p-5">
          {children}
        </div>
      </div>
    </div>
...
export default function OAuthConsent() {
      ...
      try {
        ...
        if (!data.authenticated) {
          ...
          window.location.href =
            (data.login_path || "/auth") + "?returnTo=" + encoded + "&from_url=" + encoded;
          return;
        }
        setInfo(data);
      } catch (e) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 401) {
          ...
          const encoded = encodeURIComponent(returnTo);
          window.location.href =
            ((info && info.login_path) || "/auth") + "?returnTo=" + encoded + "&from_url=" + encoded;
          return;
        }
  ...
  if (checking) {
    return (
      <ConsentLayout icon={ShieldCheck} title="Authorize access">
  ...
  if (reconnect) {
    return (
      <ConsentLayout icon={ShieldCheck} title="Reconnect required">
